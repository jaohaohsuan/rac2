...
case class QueryTemplate(name: String)
...
trait HttpQueryTemplateServiceRoute extends HttpService {
  ...
  object realmPathMatcher extends PathMatcher1[String] {
    def apply(path: Path) =
  ...
  def queryTemplateRoute = pathPrefix(realmPathMatcher ~ "_query" / "template" / Segment) { (realm, id) =>
      ...
      URI { href =>
        get {
          pathEndOrSingleSlash {
            ...
            complete(JsonCollection(href, links, List.empty, List.empty, QueryTemplate("")))
          } ~
          ...
          post {
            //save as new
            entity(as[QueryTemplate]) { template =>
              val newId = template.hashCode.toString
              respondWithHeader(RawHeader("Location", href.resolve(newId).toString)) {
                complete(Created)
              }
            }
          } ~
